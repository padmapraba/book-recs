...
if not os.path.exists('/Users/padmaprabagaran/dev/book-recs/data/data.csv'):
    ...
    book_data.to_csv('/Users/padmaprabagaran/dev/book-recs/data/data.csv',index=False)

books = pd.read_csv('/Users/padmaprabagaran/dev/book-recs/data/book_urls.csv')
book_data = pd.read_csv('/Users/padmaprabagaran/dev/book-recs/data/data.csv')
...
for i in range(len(book_data), len(books)):
    ...
    try:
        ...
        if i % save_every == 0:
            # book_data.concat(pd.DataFrame.from_dict(book_rows)).to_csv(
            #     'data/data.csv', index=False)
            pd.DataFrame(book_rows).to_csv('data/data.csv', mode='a', header=False, index=False)
            book_rows = []
    except:
        # book_data.concat(pd.DataFrame.from_dict(book_rows)).to_csv(
        #     'data/data.csv', index=False)
